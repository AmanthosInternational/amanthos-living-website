...
### Segment 3: amanthos-living-website (GBAL, GNBE, NYAL): Deep Link mit Haus, Kurtaxe-Zeile
...
- **Datei-Scope (exklusiv):**
  ...
  ```
- **Auftrag:** Wie Segment 1, zusätzlich: `property` ist Pflicht und wählt das Haus über
  `selectLocation()`; Belegungsgrenze aus `MAX_GUESTS` je Haus; `adsEvent` mit
  `id = searchParams.propertyId`. Neu ist die Kurtaxe-Anzeige für Angebote mit
  `cityTax.amount > 0 && !cityTax.included` (NYAL): Zeile auf der Karte, Bruttosumme, Hinweis
  „separat belastet", dieselbe Zeile in der Preiszusammenfassung vor den Gästeangaben;
  Microdata-`price` = Brutto. `totalAmount` und der Zahlungsweg bleiben unverändert (der
  Einzug der NYAL-Kurtaxe läuft weiter über den bestehenden Folio-Weg). Texte über
  `window.t` mit englischem Fallback und den Schlüsseln aus K7.
- **Akzeptanzkriterien:**
  ```bauplan-kriterien segment=3
  - [ ] node --test tests/ Exit 0, Testzahl zitiert; alle Faelle aus tests/fixtures/deeplink-cases.json laufen durch parse(), darunter der Living-Fall ohne property (search null)
  - [ ] Harness: http://localhost:8080/?property=NYAL&arrival=<heute+30>&departure=<heute+32>&adults=2&children=0&lang=fr&utm_source=google&utm_medium=organic&utm_campaign=hotel-fbl zeigt OHNE Klick die NYAL-Angebote aus dem Fixture, Standortlabel Nyon, html[lang=fr]; jede Karte zeigt die Kurtaxe-Zeile (18.00 im Fixture) und den Bruttobetrag; meta[itemprop=identifier][content=NYAL]; meta[itemprop=price] gleich dem Brutto, genau eine priceComponent (City tax, priceComponentType GenericTax); checkinTime/checkoutTime als DateTime; DevTools-Ausgabe zitiert
  - [ ] ?property=GBAL mit offers-gbal.json (cityTax 0): keine Kurtaxe-Zeile, keine priceComponent, price gleich Zimmerpreis; ?property=PRZA und fehlendes property: keine Autosuche, kein Fehler
  - [ ] Nach Auswahl eines NYAL-Angebots zeigt die Preiszusammenfassung Zimmer, Kurtaxe (separat belastet) und Gesamt; der an den Mock gesendete totalAmount entspricht weiterhin Zimmer plus Extras ohne Kurtaxe (Body zitiert)
  - [ ] ?lang=ko fuehrt zu html[lang=ko] (unterstuetzt); ?lang=pt fuehrt zu html[lang=en]
  - [ ] ?fixture=empty: SoldOut-Knoten ohne Preis
  - [ ] Mock-Body enthaelt bookingSource und campaign; __gtagCalls wie Segment 1 mit id gleich dem Property-Code
  - [ ] plausibleEvent, index.html, locales/, css/ unveraendert; Produktivcode-Diff hoechstens 400 Zeilen; kein Datei-Diff ausserhalb des Scopes; Plan-Abgleich in jedem Commit
  ```
- **Out-of-Scope:** Online-Einzug der NYAL-Kurtaxe (Backend, Inhaberentscheid),
  `chat.js`, `locales/` (Schlüssel setzt die Verdrahtung), `index.html`.
- **Testplan:** wie Segment 1, zusätzlich mit beiden Fixtures.
- **Hängt ab von:** Segment 0.
- **Diff geschätzt:** ~340 Zeilen Produktivcode (Kurtaxe-Anzeige ~50 zusätzlich). Das
  ist das engste Segment; bei Überschreitung anhalten (vorgesehener Schnitt: Kurtaxe-Anzeige
  als eigenes Folge-Segment 6 im selben Repo, dann ohne Microdata-Bruttopreis für NYAL bis
  dahin).
